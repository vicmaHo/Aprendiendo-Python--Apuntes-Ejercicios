...
def AutoPartes(ventas: list):
    ...
    for i in range(len(ventas)):
        dictionary[i] = tuple(ventas[i])
    return dictionary
    
def consultaRegistro(ventas: dict, idProducto: int):
    noExiste = True
    for i in range(len(ventas)):
        if idProducto == ventas[i][0]:
            print(f"Producto consultado : {idProducto}  Descripción  {ventas[i][1]}  #Parte  {ventas[i][2]}  Cantidad vendida  {ventas[i][3]}  Stock  {ventas[i][4]}  Comprador {ventas[i][5]}  Documento  {ventas[i][6]}  Fecha Venta  {ventas[i][7]}")
            noExiste = False
    if noExiste:
